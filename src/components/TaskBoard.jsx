...
const TaskBoard = () => {
  ...
  const handleDragEnd = (result) => {
    ...
    saveTasksToStorage(updatedTasks);  // Save immediately after drag
  };

  const handleAddTask = (newTask) => {
    const { status } = newTask;  // Use the actual selected status
    const updatedTasks = {
        ...tasks,
  ...
  const handleUpdateTask = (updatedTask) => {
    const oldStatus = editTask?.status;   // Previous status before editing
    const newStatus = updatedTask.status;  // New status after editing

    const updatedTasks = { ...tasks };

    if (oldStatus === newStatus) {
        // ✅ Same column - simple update
        updatedTasks[newStatus] = updatedTasks[newStatus].map(task =>
            task.id === updatedTask.id ? updatedTask : task
        );
    } else {
        // ✅ Status changed - move across columns
